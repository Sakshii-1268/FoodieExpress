...
const Orders = () => {
  const { restaurantOrder, getRestaurantOrders, updateRestaurantOrder } =
    useRestaurantStore();
  const handleStatusChanged = async (id: string, status: string) => {
        ...
        {restaurantOrder.map((order) => (
          <div className="flex flex-col md:flex-row justify-between items-start sm:items-center bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6 sm:p-8 border-gray-200 dark:border-gray-700">
            ...
            <div className="w-full sm:w-1/3">
              ...
              <Select onValueChange={(newStatus) => handleStatusChanged(order._id, newStatus)} defaultValue={order.status}>
                <SelectTrigger>
